...
class SimMcpClient:
    def __init__(self, *, settings: Settings | None = None) -> None:
        ...
        script = repo_root / "mcp_servers" / "sim_mcp" / "server.py"
    ...
    def list_tools(self) -> dict[str, Any]:
        return self._rpc.call("tools/list", {}, timeout_s=self._settings.timeout_sim_s)

    def run(self, spec: dict[str, Any]) -> ToolResponse[Any]:
        try:
            result = self._rpc.call(
                "tools/call",
                {"name": "sim_run", "arguments": spec},
                timeout_s=self._settings.timeout_sim_s,
            )
