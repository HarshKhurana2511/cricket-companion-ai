...
class WebMcpClient:
    def __init__(self, *, settings: Settings | None = None) -> None:
        ...
        script = repo_root / "mcp_servers" / "web_mcp" / "server.py"
    ...
    def list_tools(self) -> dict[str, Any]:
        return self._rpc.call("tools/list", {}, timeout_s=self._settings.timeout_web_s)

    def search(self, spec: dict[str, Any]) -> ToolResponse[Any]:
        try:
            result = self._rpc.call(
                "tools/call",
                {"name": "web_search", "arguments": spec},
                timeout_s=self._settings.timeout_web_s,
            )
    ...
    def fetch(self, spec: dict[str, Any]) -> ToolResponse[Any]:
        try:
            result = self._rpc.call(
                "tools/call",
                {"name": "web_fetch", "arguments": spec},
                timeout_s=self._settings.timeout_web_s,
            )
        except TimeoutError as exc:
            return ToolResponse.failure(ErrorCode.TIMEOUT, str(exc), meta=ToolMeta())
    ...
    def espn_ingest(self, spec: dict[str, Any]) -> ToolResponse[Any]:
        try:
            result = self._rpc.call(
                "tools/call",
                {"name": "espn_ingest", "arguments": spec},
                timeout_s=self._settings.timeout_web_s,
            )
        except TimeoutError as exc:
            return ToolResponse.failure(ErrorCode.TIMEOUT, str(exc), meta=ToolMeta())
