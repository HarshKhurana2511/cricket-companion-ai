...
class FantasyMcpClient:
    def __init__(self, *, settings: Settings | None = None) -> None:
        self._settings = settings or get_settings()
        repo_root = Path(__file__).resolve().parents[2]
        script = repo_root / "mcp_servers" / "fantasy_mcp" / "server.py"
    ...
    def list_tools(self) -> dict[str, Any]:
        return self._rpc.call("tools/list", {}, timeout_s=self._settings.timeout_fantasy_s)

    def optimize(self, spec: dict[str, Any]) -> ToolResponse[Any]:
        try:
            result = self._rpc.call(
                "tools/call",
                {"name": "fantasy_optimize", "arguments": spec},
                timeout_s=self._settings.timeout_fantasy_s,
            )
